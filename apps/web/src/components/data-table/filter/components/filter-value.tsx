...
function __FilterValue<TData, TType extends ColumnDataType>({ filter, column, actions, strategy, locale }: FilterValueProps<TData, TType>) {
    return (
        <Popover>
...
export function FilterValueDisplay<TData, TType extends ColumnDataType>({ filter, column, actions, locale = "en" }: FilterValueDisplayProps<TData, TType>) {
    switch (column.type) {
        case "option":
            return (
        ...
        case "date":
            return <FilterValueDateDisplay filter={filter as FilterModel<"date">} column={column as Column<TData, "date">} actions={actions} locale={locale} />;
        case "text":
            return <FilterValueTextDisplay filter={filter as FilterModel<"text">} column={column as Column<TData, "text">} actions={actions} locale={locale} />;
        case "number":
            return (
...
export function FilterValueOptionDisplay<TData>({ filter, column, actions, locale = "en" }: FilterValueDisplayProps<TData, "option">) {
...
export function FilterValueMultiOptionDisplay<TData>({ filter, column, actions, locale = "en" }: FilterValueDisplayProps<TData, "multiOption">) {
    const options = useMemo(() => column.getOptions(), [column]);
    const selected = options.filter((o) => filter.values.includes(o.value));
...
export function FilterValueDateDisplay<TData>({ filter, column, actions, locale = "en" }: FilterValueDisplayProps<TData, "date">) {
...
export function FilterValueTextDisplay<TData>({ filter, column, actions, locale = "en" }: FilterValueDisplayProps<TData, "text">) {
    if (!filter) return null;
...
export function FilterValueNumberDisplay<TData>({ filter, column, actions, locale = "en" }: FilterValueDisplayProps<TData, "number">) {
...
export function FilterValueOptionController<TData>({ filter, column, actions, locale = "en" }: FilterValueControllerProps<TData, "option">) {
...
export function FilterValueMultiOptionController<TData>({ filter, column, actions, locale = "en" }: FilterValueControllerProps<TData, "multiOption">) {
    // Compute initial options once per mount
    const initialOptions = useMemo(() => {
        const counts = column.getFacetedUniqueValues();
        return column.getOptions().map((o) => {
...
export function FilterValueDateController<TData>({ filter, column, actions }: FilterValueControllerProps<TData, "date">) {
    ...
    return (
        <Command>
            <CommandList className="max-h-fit">
                <CommandGroup>
                    <div>
                        <Calendar initialFocus mode="range" defaultMonth={date?.from} selected={date} onSelect={changeDateRange} numberOfMonths={1} />
                    </div>
                </CommandGroup>
            </CommandList>
        </Command>
...
export function FilterValueTextController<TData>({ filter, column, actions, locale = "en" }: FilterValueControllerProps<TData, "text">) {
    ...
    return (
        <Command>
            <CommandList className="max-h-fit">
                <CommandGroup>
                    <CommandItem>
                        <DebouncedInput placeholder={t("search", locale)} autoFocus value={filter?.values[0] ?? ""} onChange={changeText} />
                    </CommandItem>
                </CommandGroup>
            </CommandList>
        </Command>
...
export function FilterValueNumberController<TData>({ filter, column, actions, locale = "en" }: FilterValueControllerProps<TData, "number">) {
            ...
            <CommandList className="w-[300px] px-2 py-2">
                <CommandGroup>
                    <div className="flex w-full flex-col">
                        <Tabs value={isNumberRange ? "range" : "single"} onValueChange={(v) => changeType(v as "single" | "range")}>
                            ...
                            <TabsContent value="range" className="mt-4 flex flex-col gap-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <DebouncedInput type="number" value={values[0]} onChange={(v) => changeMinNumber(Number(v))} />
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <span className="text-xs font-medium">{t("max", locale)}</span>
                                        <DebouncedInput type="number" value={values[1]} onChange={(v) => changeMaxNumber(Number(v))} />
                                    </div>
                                </div>
                            </TabsContent>
                        </Tabs>
