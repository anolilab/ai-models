 ...
 */
export function createOptimizedRowSelectionHandler<TData>(
  data: TData[],
  idField: keyof TData,
  onSelectionChange: (selectedIds: Set<string>) => void
) {
  return useCallback((updaterOrValue: Record<string, boolean> | ((prev: Record<string, boolean>) => Record<string, boolean>)) => {
...
export function useBatchedState<T>(initialState: T) {
  ...
  const batchedSetState = useCallback((updater: T | ((prev: T) => T)) => {
    ...
    batchRef.current = typeof updater === 'function' 
      ? (updater as (prev: T) => T)(batchRef.current ?? state)
      : updater;
 ...
 */
export function createOptimizedSortFunction<T>(
  accessorKey: keyof T,
 ...
 */
export function useDebouncedSorting<T>(
  data: T[],
 ...
 */
export function createOptimizedColumnSorting<T>(
  columns: Array<{ accessorKey: keyof T; id: string; meta?: { sortType?: string } }>
) {
