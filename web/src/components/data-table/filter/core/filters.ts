...
export const createColumnConfigHelper = <TData>(): FluentColumnConfigHelper<TData> => {
    return {
...
export function getColumnOptions<TData, TType extends ColumnDataType, TVal>(
    ...
    if (column.orderFn) {
        models = models.sort((m1, m2) => column.orderFn!(m1 as ElementType<NonNullable<TVal>>, m2 as ElementType<NonNullable<TVal>>));
    }
...
export const getColumnValues = <TData, TType extends ColumnDataType, TVal>(column: ColumnConfig<TData, TType, TVal>, data: TData[]) => {
    ...
    if (column.options) {
        return raw.map((v) => column.options?.find((o) => o.value === v)?.value).filter((v) => v !== undefined && v !== null);
    }
...
export function getFacetedMinMaxValues<TData, TType extends ColumnDataType, TVal>(
    ...
    const values = data.flatMap((row) => column.accessor(row) as Nullable<number>).filter((v): v is number => typeof v === "number" && !Number.isNaN(v));
