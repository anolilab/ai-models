...
export const dateFilterFn = <TData>(row: Row<TData>, columnId: string, filterValue: FilterModel<"date">): boolean => {
    const value = row.getValue<Date>(columnId);

    return f_dateFilterFn(value, filterValue);
};

export const textFilterFn = <TData>(row: Row<TData>, columnId: string, filterValue: FilterModel<"text">): boolean => {
    const value = row.getValue<string>(columnId) ?? "";

    return f_textFilterFn(value, filterValue);
};

export const numberFilterFn = <TData>(row: Row<TData>, columnId: string, filterValue: FilterModel<"number">): boolean => {
