 ...
 */
async function scrapeGitHubDocs(): Promise<Model[]> {
  try {
    ...
    $('table, .model-list, .models-table').each((i, element) => {
      ...
      if (tableText.includes('model') || tableText.includes('github') || tableText.includes('copilot')) {
        console.log(`[GitHub Models] Found potential model table ${i + 1}`);
    ...
    if (models.length === 0) {
      ...
      const modelMatches = bodyText.match(/([a-zA-Z0-9\-_]+(?:-github|copilot|model)[a-zA-Z0-9\-_]*)/gi);
 ...
 */
export function transformGitHubModels(rawData: any): Model[] {
  const models: Model[] = [];
